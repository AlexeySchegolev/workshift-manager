...
export class ShiftConstraint {
  ...
  get effectiveWeight(): number {
    // Adjust weight based on priority and compliance rate
    let adjustedWeight = this.weight * this.priority;
    
    // Lower compliance = higher weight (more important to satisfy)
    if (this.complianceRate < 100) {
      adjustedWeight *= (1 + (100 - this.complianceRate) / 100);
    }
  ...
  get importanceScore(): number {
    let score = this.effectiveWeight;
    
    if (this.isHardConstraint) score *= 2;
    if (!this.isFlexible) score *= 1.5;
    if (this.priority === ConstraintPriority.MANDATORY) score *= 3;
  ...
  get priorityLabel(): string {
    const labels = ['', 'Low', 'Medium', 'High', 'Critical', 'Mandatory'];
  ...
  get isCritical(): boolean {
    return this.priority >= ConstraintPriority.CRITICAL || this.isHardConstraint;
  }
  ...
  get needsAttention(): boolean {
    return this.complianceRate < 80 || this.violationCount > 10 || this.performanceImpact > 7;
  }
  ...
  isApplicableToShiftType(shiftType: string): boolean {
    if (this.applicableShiftTypes.length > 0) {
      return this.applicableShiftTypes.includes(shiftType);
    }
    return !this.excludedShiftTypes.includes(shiftType);
  }

  isApplicableToTime(time: string): boolean {
    if (!this.effectiveStartTime || !this.effectiveEndTime) return true;
    
    const checkTime = new Date(`2000-01-01T${time}`);
    const startTime = new Date(`2000-01-01T${this.effectiveStartTime}`);
    const endTime = new Date(`2000-01-01T${this.effectiveEndTime}`);
  ...
  validateConstraint(data: any): ConstraintValidationResult {
  ...
  calculateSatisfactionScore(actualValue: any, targetValue: any): number {
    // This would contain logic to calculate how well the constraint is satisfied
