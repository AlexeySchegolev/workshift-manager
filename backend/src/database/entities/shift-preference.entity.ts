...
export class ShiftPreference {
  ...
  get isCurrentlyActive(): boolean {
    ...
    return this.status === PreferenceStatus.ACTIVE && this.isActive && !this.deletedAt;
  }
  ...
  get effectiveWeight(): number {
    // Adjust weight based on learning and satisfaction
    let adjustedWeight = this.weight * this.learningWeight;
    
    if (this.satisfactionScore !== null && this.satisfactionScore !== undefined) {
      // Lower satisfaction = higher weight (more important to fulfill)
      adjustedWeight *= (1 + (1 - this.satisfactionScore / 10));
    }
    
    return Math.min(Math.max(adjustedWeight, 1), 10);
  }

  get importanceScore(): number {
    // Combine weight, priority, and constraint type
    let score = this.effectiveWeight * this.priorityLevel;
  ...
  get displayName(): string {
    return this.preferenceName || `${this.category} preference`;
  }

  get preferenceLabel(): string {
    switch (this.preference) {
      case PreferenceType.PREFERRED:
        return 'Preferred';
      case PreferenceType.NOT_PREFERRED:
  ...
  get isHighPriority(): boolean {
    return this.priorityLevel >= 4 || this.isHardConstraint;
  }

  get needsAttention(): boolean {
    return this.fulfillmentRate < 0.5 || this.violationCount > 5;
  }
}