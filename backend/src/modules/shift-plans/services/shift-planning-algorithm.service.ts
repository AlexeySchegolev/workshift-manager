...
export class ShiftPlanningAlgorithmService {
  ...
  constructor(
    ...
    @InjectRepository(PlanningStatistics)
    private planningStatisticsRepository: Repository<PlanningStatistics>,
  ...
  async generateShiftPlan(
    ...
    try {
      // Initialize shift plan structure
      ...
      this.calculateFinalStatistics(combinedShiftPlan, employees, finalAvailabilityMap, statistics);
  ...
  /**
   * Calculate comprehensive planning statistics
   */
  calculatePlanningStatistics(
    ...
    for (const dayKey in shiftPlan) {
      ...
      for (const shiftName in dayPlan) {
        ...
        shiftDistribution[shiftName] = (shiftDistribution[shiftName] || 0) + employeeIds.length;
      }
    }
    ...
    for (const employee of employees) {
      ...
      if (availability) {
        employeeUtilization[employee.id] = {
          totalHours: availability.totalHoursAssigned,
          shiftsCount: availability.shiftsAssigned.length,
          workloadPercentage: availability.workloadPercentage,
          saturdaysWorked: availability.saturdaysWorked
        };
      }
    }

    const coveragePercentage = this.calculateCoveragePercentage(shiftPlan);
    const totalEmployeesInvolved = Object.keys(employeeUtilization).length;

    const statistics = new PlanningStatistics();
    statistics.totalShiftsPlanned = totalShifts;
    statistics.totalHoursPlanned = totalHours;
    statistics.totalEmployeesInvolved = totalEmployeesInvolved;
    statistics.coveragePercentage = coveragePercentage;
    statistics.employeeUtilization = employeeUtilization;
    statistics.shiftDistribution = shiftDistribution;
